...
use App\Models\AkunPanenPoinV4;
use App\Models\User;
use App\Models\UserPanenPoinV4;
use Carbon\Carbon;
...
class PanenPoinV4Controller extends Controller
{
    private const PROGRAM_PERIOD_START = '2026-09-01';
    private const PROGRAM_PERIOD_END = '2026-10-09';
    private const PROGRAM_PERIOD_LABEL = '01 Sep 2026 - 09 Okt 2026';

    private const PREVIOUS_PERIOD_START = '2026-07-01';
    private const PREVIOUS_PERIOD_END = '2026-08-10';
    private const PREVIOUS_PERIOD_LABEL = '01 Jul 2026 - 10 Agu 2026';

    public function index()
    {
        logUserLogin();
        return view('panenpoinv2.inputdatapoin', [
            'routePrefix' => 'panenpoinv4',
            'programLabel' => 'Panen Poin V4',
        ]);
    }
    ...
    public function report()
    {
        ...
        return view('panenpoinv2.reportpoin', [
            ...
            'pointPeriodDescription' => 'Poin dihitung untuk periode ' . self::PROGRAM_PERIOD_LABEL . ', tanpa membawa saldo poin V3.',
            'routePrefix' => 'panenpoinv4',
            'programLabel' => 'Panen Poin V4',
        ]);
    }
    ...
    public function getAkunData(Request $request)
    {
        ...
        $data = $query->select('id', 'nama_akun', 'email_client', 'user_id', 'created_at')
            ...
            ->get()
            ->map(function ($item) {
                $nomorHp = DB::table('user_panen_poin_v4')
    ...
    {
        $request->merge(['akun_myads_pelanggan' => strtolower(trim((string) $request->akun_myads_pelanggan))]);

        $request->validate([
            'nama_pelanggan' => 'required|string|max:255',
            'akun_myads_pelanggan' => [
                'required', 'email', 'max:255',
                Rule::unique('akun_panen_poin_v4', 'email_client')
                    ->where(fn ($query) => $query->where('user_id', '<>', Auth::id())),
            ],
            'nomor_hp_pelanggan' => 'required|string|max:20',
        ], [
            'akun_myads_pelanggan.unique' => 'Email pelanggan sudah terdaftar pada pengguna lain di Panen Poin V4.',
        ]);
        ...
        DB::beginTransaction();
        try {
            $existing = AkunPanenPoinV4::where('user_id', Auth::id())
                ->where('email_client', $emailClient)
                ->first();

            $isNewAccount = false;
            if (!$existing) {
                $akun = AkunPanenPoinV4::create([
                    'user_id' => Auth::id(),
                    'nama_akun' => $request->nama_pelanggan,
                    'email_client' => $emailClient,
                    'password' => bcrypt('123456'),
                    'source' => 'user_panen_poin_v4',
                ]);

                UserPanenPoinV4::create([
                    'user_id' => Auth::id(),
            ...
            return redirect()->route('panenpoinv4.index')
                ->with('success', $isNewAccount ? 'Data pelanggan berhasil disimpan!' : 'Akun sudah ada di Panen Poin V4.')
                ->with('is_existing_account', !$isNewAccount);
        } catch (\Throwable $e) {
            DB::rollBack();
            \Log::error('Gagal menyimpan peserta Panen Poin V4', ['exception' => $e]);
            return redirect()->back()->with('error', 'Data belum berhasil disimpan. Silakan coba lagi.')->withInput();
        }
    }
    ...
    public function export(Request $request)
    {
        ...
        $sheet->setCellValue('A1', 'LAPORAN PANEN POIN V4 - ' . strtoupper($period['label']));
        ...
        }, 'Laporan_Panen_Poin_V4_' . str_replace(' ', '_', $period['label']) . '.xlsx');
    }

    public function refreshSummaryPanenPoinV4()
    {
        ...
        if (empty($canvasserIds)) {
            return response()->json(['success' => true, 'message' => 'Summary Panen Poin V4 berhasil direfresh.']);
        }

        $manualInputs = UserPanenPoinV4::whereIn('user_id', $canvasserIds)
        ...
        $akunByEmail = AkunPanenPoinV4::query()
            ->select('id', 'email_client')
            ->get()
            ->keyBy(fn ($row) => strtolower(trim($row->email_client)));

        $redeemByAkunId = DB::table('prize_redeems_v4')
        ...
        $previousSummaryByUser = DB::table('summary_panen_poin_v4')
            ->select(
                'user_id',
                'email_client',
        ...
        $existingSummary = DB::table('summary_panen_poin_v4')
            ...
            foreach ($clients as $client) {
                ...
                if ($totalSettlement == 0 && $poinAkumulasi == 0 && $poinPackage == 0) {
                    ...
                    if ($existingSummary->has($existingKey)) {
                        DB::table('summary_panen_poin_v4')
                ...
                if ($existing) {
                    DB::table('summary_panen_poin_v4')->where('id', $existing->id)->update($payload);
                } else {
                    $payload['created_at'] = now();
                    $insertedId = DB::table('summary_panen_poin_v4')->insertGetId($payload);
    ...
    public function redeemPrize(Request $request)
    {
        ...
        $summary = DB::table('summary_panen_poin_v4')
            ...
            DB::table('prize_redeems_v4')->insert([
                'user_id' => $akun->id,
                'prize_id' => $prize->id,
    ...
    {
        $totalRedeem = (int) DB::table('prize_redeems_v4')
            ->where('user_id', $akunId)
            ->whereBetween(DB::raw('DATE(created_at)'), [$period['start']->toDateString(), $period['end']->toDateString()])
            ->sum('point_used');

        $akun = AkunPanenPoinV4::findOrFail($akunId);
        $summaries = DB::table('summary_panen_poin_v4')
            ->whereRaw('LOWER(TRIM(email_client)) = ?', [strtolower(trim($akun->email_client))])
            ->whereDate('period_start', $period['start']->toDateString())
            ->whereDate('period_end', $period['end']->toDateString())
            ->get();
        ...
        foreach ($summaries as $summary) {
            ...
            DB::table('summary_panen_poin_v4')->where('id', $summary->id)->update([
    ...
    protected function calculateData($periodStart = null): array
    {
        ...
        $query = DB::table('summary_panen_poin_v4')
            ->select(
                'summary_panen_poin_v4.nama_canvasser',
                'summary_panen_poin_v4.email_client',
                'akun_panen_poin_v4.id as akun_id',
                'summary_panen_poin_v4.nomor_hp_client',
                'summary_panen_poin_v4.source',
                DB::raw('CAST(summary_panen_poin_v4.total_settlement AS DECIMAL(15,2)) as total_settlement_raw'),
                'summary_panen_poin_v4.poin_bulan_ini',
                'summary_panen_poin_v4.poin_akumulasi',
                'summary_panen_poin_v4.poin',
                'summary_panen_poin_v4.poin_package',
                DB::raw('COALESCE(summary_panen_poin_v4.poin_redeem, 0) as poin_redeem'),
                DB::raw('(summary_panen_poin_v4.poin - COALESCE(summary_panen_poin_v4.poin_redeem, 0) + COALESCE(summary_panen_poin_v4.poin_package, 0)) as poin_sisa'),
                'summary_panen_poin_v4.remark',
                'summary_panen_poin_v4.periode_label as bulan'
            )
            ->join('akun_panen_poin_v4', 'summary_panen_poin_v4.email_client', '=', 'akun_panen_poin_v4.email_client')
            ->leftJoin('mitra_sbp', 'summary_panen_poin_v4.email_client', '=', 'mitra_sbp.email_myads')
            ->whereNull('mitra_sbp.id')
            ->whereDate('summary_panen_poin_v4.period_start', $period['start']->toDateString())
            ->whereDate('summary_panen_poin_v4.period_end', $period['end']->toDateString());

        if (Auth::user()->role === 'cvsr') {
            $query->where('summary_panen_poin_v4.user_id', Auth::id());
        }

        if (request()->filled('source')) {
            $query->where('summary_panen_poin_v4.source', request()->source);
        }

        if (request()->filled('remark')) {
            $query->where('summary_panen_poin_v4.remark', request()->remark);
        }

        return $query->orderByRaw('(summary_panen_poin_v4.poin_package + summary_panen_poin_v4.poin - COALESCE(summary_panen_poin_v4.poin_redeem, 0)) DESC')
            ->get()
            ->map(function ($item) {
                return [
    ...
    protected function buildReportByRole(string $role, $periodStart = null)
    {
        ...
        $query = DB::table('users')
            ->select(
                ...
                DB::raw('COALESCE(COUNT(CASE WHEN akun_panen_poin_v4.email_client IS NOT NULL AND mitra_sbp.id IS NULL THEN summary_panen_poin_v4.id END), 0) as jumlah_terdaftar'),
                DB::raw('COALESCE(SUM(CASE WHEN akun_panen_poin_v4.email_client IS NOT NULL AND mitra_sbp.id IS NULL AND (COALESCE(summary_panen_poin_v4.poin, 0) > 0 OR COALESCE(summary_panen_poin_v4.poin_package, 0) > 0) THEN 1 ELSE 0 END), 0) as jumlah_akun_punya_poin'),
                DB::raw('COALESCE(SUM(CASE WHEN akun_panen_poin_v4.email_client IS NOT NULL AND mitra_sbp.id IS NULL THEN summary_panen_poin_v4.poin ELSE 0 END), 0) as jumlah_poin')
            )
            ->where('users.role', $role)
            ->leftJoin('summary_panen_poin_v4', function ($join) use ($period) {
                $join->on('summary_panen_poin_v4.user_id', '=', 'users.id')
                    ->whereDate('summary_panen_poin_v4.period_start', $period['start']->toDateString())
                    ->whereDate('summary_panen_poin_v4.period_end', $period['end']->toDateString());
            })
            ->leftJoin('akun_panen_poin_v4', 'summary_panen_poin_v4.email_client', '=', 'akun_panen_poin_v4.email_client')
            ->leftJoin('mitra_sbp', 'summary_panen_poin_v4.email_client', '=', 'mitra_sbp.email_myads')
            ->groupBy('users.id', 'users.name')
            ->orderByRaw('COALESCE(SUM(CASE WHEN akun_panen_poin_v4.email_client IS NOT NULL AND mitra_sbp.id IS NULL THEN summary_panen_poin_v4.poin ELSE 0 END), 0) DESC');
    ...
    protected function refreshSummaryForSingleUser($userId, $emailClient): void
    {
        ...
        $clientData = UserPanenPoinV4::where('user_id', $userId)
        ...
        $akun = AkunPanenPoinV4::whereRaw('LOWER(TRIM(email_client)) = ?', [$email])->first();
        $poinRedeem = $akun ? (int) DB::table('prize_redeems_v4')
        ...
        $existing = DB::table('summary_panen_poin_v4')
            ->where('user_id', $userId)
            ->where('email_client', $email)
            ->whereDate('period_start', $period['start']->toDateString())
            ->whereDate('period_end', $period['end']->toDateString())
        ...
        if ($existing) {
            ...
            $payload['created_at'] = now();
            DB::table('summary_panen_poin_v4')->insert($payload);
        }
    }
    ...
    protected function resolveClientPool(int $userId): array
    {
        return $this->resolveClientPoolFromCollections(
            UserPanenPoinV4::where('user_id', $userId)->get(),
    ...
    protected function resolveClientPoolFromCollections($manualRows, $leadRows): array
    {
        ...
        return collect($clients)
            ->reject(function ($client) use ($leadsMap) {
                return $client['source'] === 'user_panen_poin_v4' && isset($leadsMap[$client['email']]);
            })
    ...
    {
        $isManualInput = DB::table('user_panen_poin_v4')
            ->where('user_id', $userId)
            ->where('akun_myads_pelanggan', $email)
            ->exists();

        if ($isManualInput) {
            return 'user_panen_poin_v4';
        }
