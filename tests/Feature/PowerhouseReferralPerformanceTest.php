...
class PowerhouseReferralPerformanceTest extends TestCase
{
    public function test_bounded_queries_preserve_totals_and_month_boundaries(): void
    {
        ...
        DB::statement('CREATE TABLE leads_master (id INTEGER, user_id INTEGER, email TEXT, created_at TEXT)');
        DB::statement('CREATE TABLE bookings (nama TEXT, tanggal TEXT)');
        DB::statement('CREATE TABLE data_registarsi_status_approveorreject (email TEXT, status TEXT, tanggal_approval_aktivasi TEXT)');
        DB::statement('CREATE TABLE report_balance_top_up (id INTEGER, email_client TEXT, amount REAL, tgl_transaksi TEXT, payment_method_name TEXT)');
        DB::statement('CREATE TABLE saldo_transfer (id INTEGER, email_client TEXT, amount REAL, tgl_transaksi TEXT)');
