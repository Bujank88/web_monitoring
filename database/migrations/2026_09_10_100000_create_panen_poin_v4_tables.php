...
return new class extends Migration
{
    ...
    {
        Schema::create('user_panen_poin_v4', function (Blueprint $table) {
            $table->id();
        ...
        if (Schema::hasTable('user_panen_poin_v3')) {
            DB::statement("
                INSERT INTO user_panen_poin_v4 (user_id, nama_pelanggan, akun_myads_pelanggan, nomor_hp_pelanggan, created_at, updated_at)
                SELECT user_id, nama_pelanggan, akun_myads_pelanggan, nomor_hp_pelanggan, created_at, updated_at
                FROM user_panen_poin_v3
            ");
        }

        if (Schema::hasTable('akun_panen_poin_v3')) {
            DB::statement("
                INSERT INTO akun_panen_poin_v4 (uuid, user_id, nama_akun, email_client, password, source, created_at, updated_at)
                SELECT uuid, user_id, nama_akun, email_client, password, CASE WHEN source IN ('user_panen_poin_v2', 'user_panen_poin_v3') THEN 'user_panen_poin_v4' ELSE source END, created_at, updated_at
                FROM akun_panen_poin_v3
            ");
        }
    }
    ...
    public function down(): void
    {
        ...
        Schema::dropIfExists('user_panen_poin_v4');
    }
};
