@extends('master')
@section('title') AM Referral Champion @endsection
@section('css')
<!-- Select2 CSS -->
...
{{-- <div class="row mb-3">
    <div class="col-12 d-flex justify-content-end align-items-center gap-2">
        ...
        <button type="button" id="btnSaveAMImage" class="btn btn-success" title="Save as Image" style="padding: 6px 12px; white-space: nowrap;">
            <i class="fas fa-image mr-2"></i> Save Image
        </button>
        <a href="{{ route('am.referral.export') }}" class="btn btn-success" title="Download Excel" style="padding: 6px 12px; white-space: nowrap;">
...
<div class="row mb-3">
    <div class="col-12 d-flex justify-content-end align-items-center gap-2">
        ...
        <!-- Save Image -->
        <button type="button" id="btnSaveAMImage" class="btn btn-success" title="Save as Image">
            <i class="fas fa-image mr-2"></i> Save Image
        </button>

        <!-- Download Excel -->
        <a
            {{-- href="{{ route('export.am_voucher', [
    ...
    <div class="col-12">
        <div class="card" id="powerHouseTableCard">
            <div class="card-header bg-gradient-danger text-white">
                <h4 class="mb-0"><i class="fas fa-table"></i> Report AM Referral</h4>
            </div>
            <div class="card-body">
                <div id="captureAMTable" class="table-responsive">
            <table class="table table-sm w-100 table-bordered table-hover" id="powerHouseTable" style="font-size: 13px;">
                        <thead class="table-light">
                            <tr style="background-color: #e8eaf6; font-weight: bold;">
                                <th colspan="11" style="text-align: center; padding: 10px; border-bottom: 2px solid #667eea;">Report AM Referral | <span class="displayedStartDatePH">{{ $startDate }}</span> s/d <span class="displayedEndDatePH">{{ $endDate }}</span></th>
                            </tr>
                            <tr>
                                <th style="text-align: center; width: 5%;">No</th>
                                <th style="text-align: center;">Referral Code</th>
                                <th style="text-align: center;">Team AM</th>
...
<!-- Report AM Deal Top Up & MOM -->
<div class="row mb-4">
    <div class="col-12">
        <div class="card" id="powerHousePerformanceCard">
            <div class="card-header bg-gradient-info text-white">
                <h4 class="mb-0"><i class="fas fa-chart-line"></i> Report AM Deal Top Up & MOM</h4>
            </div>
            <div class="card-body">
                <div id="captureAMPerformanceTable" class="table-responsive">
                    <table class="table table-sm w-100 table-bordered table-hover" id="powerHousePerformanceTable" style="font-size: 13px;">
                        <thead class="table-light">
                            <tr style="background-color: #e8eaf6; font-weight: bold;">
                                <th colspan="14" style="text-align: center; padding: 10px; border-bottom: 2px solid #667eea;">Data Topup & MOM AM | <span class="displayedStartDatePH">{{ $startDate }}</span> s/d <span class="displayedEndDatePH">{{ $endDate }}</span></th>
                            </tr>
                            <tr>
                                <th rowspan="2" style="text-align: center; width: 5%;">No</th>
                                <th rowspan="2" style="text-align: center;">Team AM</th>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-sm w-100 table-bordered table-hover" id="powerHouseSaldoTransferTable" style="font-size: 13px;">
                        <thead class="table-light">
                            <tr style="background-color: #e8eaf6; font-weight: bold;">
                                <th colspan="7" style="text-align: center; padding: 10px; border-bottom: 2px solid #667eea;">Detail Transfer Saldo AM | <span class="displayedStartDatePH">{{ $startDate }}</span> s/d <span class="displayedEndDatePH">{{ $endDate }}</span></th>
                            </tr>
                            <tr>
                                <th style="text-align: center; width: 5%;">No</th>
                                <th style="text-align: center;">Team AM</th>
    ...
    $(document).ready(function() {
        ...
        var table = $('#powerHouseTable').DataTable({
            ...
            columns: [
                ...
                { data: 'team_am', name: 'team_am', className: 'text-center' },
        ...
        var performanceTable = $('#powerHousePerformanceTable').DataTable({
            ...
            ordering: false,
            ajax: {
                url: "{{ route('am.referral.deal-data') }}",
                type: 'GET',
                data: function(d) {
                    d.start_date = $('#startDatePH').val();
                    d.end_date = $('#endDatePH').val();
        ...
        var saldoTransferTable = $('#powerHouseSaldoTransferTable').DataTable({
            ...
            ordering: false,
            ajax: {
                url: "{{ route('am.referral.saldo-data') }}",
                type: 'GET',
                data: function(d) {
                    d.start_date = $('#startDatePH').val();
                    d.end_date = $('#endDatePH').val();
        ...
        $('#btnExportAM').on('click', function (e) {
            e.preventDefault();

            const baseUrl = "{{ route('am.referral.export') }}";
        ...
        document.getElementById('btnSaveAMImage').addEventListener('click', function () {
            html2canvas(document.getElementById('captureAMTable'), { 
                scale: 2,
                allowTaint: true,
                useCORS: true
            })
                .then(canvas => {
                    ...
                    link.download = 'am-report-' + new Date().getTime() + '.png';
